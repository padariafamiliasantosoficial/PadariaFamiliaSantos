    ...
    produtosFiltrados.forEach(produto => {
        ...
        card.addEventListener('click', (e) => {
            if (e.target.tagName !== 'BUTTON' && e.target.tagName !== 'INPUT') {
                exibirProdutoDetalhes(produto.id); // Chama a função em vez de mudar a URL
            }
        });
...
function exibirProdutoDetalhes(id) {
    ...
    if (produto && infoProduto) {
        ...
        // Adiciona classes para ajustar o layout
...
window.addEventListener('popstate', () => {
    ...
    if (window.location.pathname.includes('Produtos')) {
        ...
        let produto = null;
        // Prioriza slug, mas mantém suporte para id (para URLs antigas)
        if (productSlug) {
            produto = produtos.find(p => p.slug === productSlug);
        } else if (productId) {
            produto = produtos.find(p => p.id == productId);
